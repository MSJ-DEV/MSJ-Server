...
module.exports = {
  createAdmin: (req, res) => {
    var passwordHashed = crypto
      ...
      .digest("hex");
    console.log(repeatepasswordHshed);
    var repeatepasswordHshed = crypto
      .createHash("sha256")
      .update(req.body.repeatepassword, "utf8")
      .digest("hex");
  ...
  UpdateAdmin: (admin, id) => {
    var passwordHashed = crypto
      .createHash("sha256")
      .update(admin.password, "utf8")
      .digest("hex");
    return new Promise((resolve, reject) => {
      connection.query(
        `UPDATE admin SET Firstname=?, Lastname=?, email=?, Address=?, numberPhone=?,  password=?,repeatepassword="${passwordHashed}", image=?, country=?, Zip=? WHERE id=?`,
        [
          admin.Firstname,
          admin.Lastname,
          admin.email,
